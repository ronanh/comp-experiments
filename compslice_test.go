...
func testCompressedBuffer[T compexperiments.PackType](bufs [][]T, withMinMax bool, t *testing.T) {
...
func testCompressBufferIntFull[T compexperiments.PackType](t *testing.T) {
...
func genBuf[T compexperiments.PackType](start, size int) []T {
	buf := make([]T, size)
	for i := 0; i < size; i++ {
		buf[i] = T(start + i)
	}
